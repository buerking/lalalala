        ...
        try:
            ...
            for idx, product in enumerate(products, 1):
                ...
                try:
                    ...
                    if not callback_ok:
                        self.logger.error("加购回调 addedCartCallbackSimple 未成功（未配置或接口返回 Success=false）")
                        messages.append("加购回调 addedCartCallbackSimple 未成功或未调用（未配置/接口返回失败）")
                        self._handle_order_issue(order, messages, reason="加购回调失败")
                        return False, self._make_order_summary(order, failure_reason="加购回调 addedCartCallbackSimple 未成功或未调用")
